...
class DownThread(Thread):
    def __init__(self, textinput, osc_client, option='part_of_speech'):
    ...
    def run(self):
        ...
        if(self.option == 'part_of_speech'):
            ...
            pos = nlp.pos_tag(self.textinput);
            #print 'Part of Speech:', nlp.pos_tag(pos)
            
            done = False
            grp = ""
            
            for x in range(0, len(pos)):
                self.osc_client.send('/token/'+pos[x][1]+' '+ unicode(pos[x][0]).encode('utf-8'))
                
                if (pos[x][1] == "NOUN" or pos[x][1] == "PROPN"):
                    #print "NOUN " + pos[x][0]
                    if(grp == ""):
                        grp = pos[x][0]
                    else:
            ...
            print 'Constituency Parsing:', nlp.parse(self.textinput)
        elif(self.option == 'dependency_parsing'):
            print 'Dependency Parsing:', nlp.dependency_parse(self.textinput)
        elif(self.option == 'tokenize'):
            print 'Tokenize:', nlp.word_tokenize(self.textinput)
        elif(self.option == 'coreference'):
            print 'Coreference:', nlp.coref(self.textinput)
...
class StanfordNLP:
    ...
    def search(self, message):
        ...
        if(self.option == 'all'):
            thd = DownThread(message, self.osc_client, 'part_of_speech');
            thd.start();
            thd2 = DownThread(message, self.osc_client, 'named_entities');
            thd2.start();
        else:
            thd = DownThread(message, self.osc_client, self.option);
            thd.start();
